...
/* -------------------------------------------------------------------------- */
/*                     DYNAMIC TABLE FIELD CONFIGURATION                     */
/* -------------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------- */
/*                    GET DYNAMIC TABLE FIELD CONFIGURATION                  */
/* -------------------------------------------------------------------------- */

/**
 * Returns the actual is_required value configured for
 * the dynamic table.
 *
 * field_templates.is_required = template default
 * dynamic_table_fields.is_required = admin-selected value
 */
export const getDynamicTableFields = async (client, dynamicTableId) => {
  const result = await client.query(
    `
    SELECT
      dtf.field_template_id,
      ft.field_key,
      dtf.is_required,
      dtf.display_order
    FROM auth.dynamic_table_fields dtf
    INNER JOIN auth.field_templates ft
      ON ft.id = dtf.field_template_id
    WHERE dtf.dynamic_table_id = $1
    ORDER BY dtf.display_order;
    `,
    [dynamicTableId],
  );
...
/* -------------------------------------------------------------------------- */
/*                    UPDATE DYNAMIC FIELD REQUIRED STATUS                   */
/* -------------------------------------------------------------------------- */

/**
 * Updates is_required for a specific field in a dynamic table.
 */
export const updateDynamicTableFieldRequired = async (
  client,
  dynamicTableId,
  fieldTemplateId,
  isRequired,
  ...
  await client.query(
    `
    UPDATE auth.dynamic_table_fields
    SET is_required = $1
    WHERE dynamic_table_id = $2
      AND field_template_id = $3;
    `,
    [Boolean(isRequired), dynamicTableId, fieldTemplateId],
  );
};
